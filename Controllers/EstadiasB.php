<?php
class EstadiasB extends Controller
{
    ...
    public function index()
    {
        ...
        $verificar = $this->model->verficarpermiso($id_user, 'Estadias Balleza');
    ...
    public function listar()
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            if ($data[$i]['estado'] == 1) {
                ...
                $data[$i]['acciones'] = '<div>
                <button class="btn btn-primary" type="button" onclick="btnEditarEB(' . $data[$i]['id'] . ');"><i class="fas fa-edit"></i></button>
                <button class="btn btn-danger" type="button"  onclick="btnEliminarEB(' . $data[$i]['id'] . ');"><i class="fas fa-trash-alt"></i></button>
                </div>';
            } else {
                $data[$i]['estado'] = '<span class="badge badge-danger">Inactivo</span>';
                $data[$i]['acciones'] = '<div>
                <button class="btn btn-success" type="button"  onclick="btnReingresarEB(' . $data[$i]['id'] . ');"><i class="fas fa-circle"></i></button>
                </div>';
            }
        }
    ...
    public function registrar()
    {
        ...
        if (empty($color_estante) || empty($id_generacion) || empty($matricula) || empty($apellido_p) || empty($apellido_m) || empty($nombre) || empty($id_carrera) || empty($codigo_estadia) || empty($nombre_proyecto) || empty($fecha_documento) || empty($nombre_empresa) || empty($tutor_academico) || empty($asesor_academico) || empty($asesor_empresarial) || empty($observaciones)) {
            ...
            if ($id == 0) {
                $data = $this->model->registrarLibro($color_estante, $id_generacion, $matricula, $apellido_p, $apellido_m, $nombre, $id_carrera, $codigo_estadia, $nombre_proyecto, $fecha_documento, $nombre_empresa, $tutor_academico, $asesor_academico, $asesor_empresarial, $observaciones, $folio);
                if ($data == "ok") {
                    $msg = "si";
                } else if ($data == "existe") {
                    $msg = " ya existe";
    ...
    {
        $ultimoCodigo = $this->model->obtenerUltimoCodigoestadia();
